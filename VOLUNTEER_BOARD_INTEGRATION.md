...
In @HnonProfitBOT, open Menu and select /board (Volunteer dates and event details). No arguments are needed. /board 2 shows the next three dates; /board show 1 opens one event. Legacy /event board commands remain supported. The Telegram menu contains only board, new, usage, status, model and stop. Profile config platforms.telegram.extra.command_menu sets max_commands=6, priority_mode=replace and that ordered priority list. Default, private-chat, group-chat, administrator and configured home-chat scopes are synchronized for default and English languages. Post-connect menu mutations remain disabled so runtime startup does not replace this menu.
