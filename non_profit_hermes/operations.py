"""
Canonical backend write operations for Non-Profit Hermes.
...
# Lazy/optional Google imports: the actual client libs are imported inside the
# functions that need them (get_creds, sheets, calendar) so that basic
# "import non_profit_hermes.operations" and --help succeed in environments
# without google-auth / google-api-python-client / PyYAML.
try:
...
class OperationsConfigurationError(RuntimeError):
    """Secret-free, actionable failure for missing runtime configuration."""


def _configured_spreadsheet_id() -> str:
    value = SPREADSHEET_ID or config.resolve_config().spreadsheet_id
    if not value:
        raise OperationsConfigurationError(
            "spreadsheet configuration is missing; set NON_PROFIT_HERMES_SPREADSHEET_ID"
        )
    return value


def _configured_calendar_id() -> str:
    value = CALENDAR_ID or config.resolve_config().calendar_id
    if not value:
        raise OperationsConfigurationError(
...
def _sheet_values(svc):
    """Validate configuration before accessing a Google Sheets service."""
    _configured_spreadsheet_id()
...
def _row_exists(svc, tab: str, id_col: str, id_value: str) -> bool:
    ...
    try:
        rows = _sheet_values(svc).get(
            spreadsheetId=_configured_spreadsheet_id(),
            range=f"{tab}!A1:Z500"
        ).execute().get("values", [])
        if not rows:
            return False
...
def _calendar_event_exists(svc_cal, title: str) -> str | None:
    ...
    try:
        events = svc_cal.events().list(
            calendarId=_configured_calendar_id(),
            q=title,
            singleEvents=True,
            maxResults=10,
        ).execute().get("items", [])
...
def get_creds(
    *,
    ...
    # Prefer module-level name (allows tests to monkeypatch operations.Credentials etc.)
    ...
    if Request is None:
        from google.auth.transport.requests import Request
    token = _configured_credentials_file(credentials_file, environ=environ)
    expected_scopes = SCOPES if scopes is None else scopes
    try:
        credential = Credentials.from_authorized_user_file(str(token), expected_scopes)
    ...
    if credential.expired and credential.refresh_token:
        refresh_and_persist_credential(credential, Request(), token, expected_scopes)
    return credential
...
def append_row(svc, tab: str, values: list[Any]) -> dict:
    ...
    body = {"values": [values]}
    result = _sheet_values(svc).append(
        spreadsheetId=_configured_spreadsheet_id(),
...
def _insert_google_calendar_event(
    ...
    *,
    ...
    return svc_cal.events().insert(calendarId=_configured_calendar_id(), body=event_body).execute()
...
def create_calendar_event_from_draft(
    ...
    *,
    ...
    values = [current.get(h, "") for h in header]
    range_end = col(len(header))
    _sheet_values(svc_sheets).update(
        spreadsheetId=_configured_spreadsheet_id(),
        range=f"CalendarLog!A{row_num}:{range_end}{row_num}",
        valueInputOption="USER_ENTERED",
        body={"values": [values]},
