"""Canonical approved-safe collection, filtering, rendering, and sync behavior.

Import and pure filtering are side-effect free. Google reads happen only through
explicitly called collection/sync functions, while public output is written only
through :func:`write_public_site` or the non-dry-run CLI path.
"""
...
from non_profit_hermes import config, models, oauth_refresh
...
refresh_and_persist_credential = oauth_refresh.refresh_and_persist_credential
refresh_credential_in_memory = oauth_refresh.refresh_credential_in_memory

_INITIAL_CONFIG = config.resolve_config()
DOCS = _INITIAL_CONFIG.public_dir
DATA = DOCS / "data"
TOKEN = _INITIAL_CONFIG.credentials_file
SCOPES = [
...
SPREADSHEET_ID = _INITIAL_CONFIG.spreadsheet_id
CALENDAR_ID = _INITIAL_CONFIG.calendar_id
...
class SyncConfigurationError(RuntimeError):
    """Secret-free, actionable failure for missing sync configuration."""


def _configured_credentials_file(
    explicit: str | Path | None = None,
    *,
    environ: Mapping[str, str] | None = None,
) -> Path:
    value = explicit or TOKEN or config.resolve_config(environ=environ).credentials_file
    if not value:
        raise SyncConfigurationError(
            "credentials configuration is missing; set NON_PROFIT_HERMES_CREDENTIALS_FILE"
        )
    return Path(value)


def _configured_spreadsheet_id(
    explicit: str | None = None,
    *,
    environ: Mapping[str, str] | None = None,
) -> str:
    value = explicit or SPREADSHEET_ID or config.resolve_config(environ=environ).spreadsheet_id
    if not value:
        raise SyncConfigurationError(
            "spreadsheet configuration is missing; set NON_PROFIT_HERMES_SPREADSHEET_ID"
        )
    return value


def _configured_calendar_id(
    explicit: str | None = None,
    *,
    environ: Mapping[str, str] | None = None,
) -> str:
    value = explicit or CALENDAR_ID or config.resolve_config(environ=environ).calendar_id
    if not value:
        raise SyncConfigurationError(
            "calendar configuration is missing; set NON_PROFIT_HERMES_CALENDAR_ID"
        )
    return value
...
def creds(
    *,
    ...
    try:
        c = Credentials.from_authorized_user_file(str(credentials_path), expected_scopes)
    except Exception as exc:
        raise SyncConfigurationError(
            "credentials are unavailable; configure a valid NON_PROFIT_HERMES_CREDENTIALS_FILE"
        ) from exc
    if c.expired and c.refresh_token:
        if persist_refresh:
            refresh_and_persist_credential(c, Request(), credentials_path, expected_scopes)
        else:
            refresh_credential_in_memory(c, Request())
    return c
...
def read_sheet_rows(
    ...
    *,
    ...
    configured_spreadsheet_id = _configured_spreadsheet_id(spreadsheet_id, environ=environ)
    result = svc.spreadsheets().values().get(
        spreadsheetId=configured_spreadsheet_id,
...
def safe_calendar_export(
    ...
    *,
    ...
    configured_calendar_id = _configured_calendar_id(calendar_id, environ=environ)
...
def collect_approved_safe_data(
    sheets_service,
    calendar_service,
    *,
    spreadsheet_id: str | None = None,
    ...
    approved_calendar = safe_calendar_export(
        tab_rows.get("CalendarLog", []),
        calendar_service,
...
def write_public_site(
    ...
    *,
    ...
    output_dir = Path(docs_dir) if docs_dir is not None else config.resolve_config(environ=environ).public_dir
    ...
    live_check = output_dir / "LIVE_CHECK_002.html"
...
def run_sync(
    ...
    *,
    ...
    configured_credentials = _configured_credentials_file(credentials_file, environ=environ)
    configured_spreadsheet_id = _configured_spreadsheet_id(spreadsheet_id, environ=environ)
    configured_calendar_id = _configured_calendar_id(calendar_id, environ=environ)
    configured_public_dir = (
        Path(public_dir)
        if public_dir is not None
        else config.resolve_config(environ=environ).public_dir
    )
    credentials = creds(
    ...
    snapshot = collect_approved_safe_data(
    ...
    if dry_run:
        ...
        return 0
    write_public_site(snapshot, docs_dir=configured_public_dir, environ=environ)
