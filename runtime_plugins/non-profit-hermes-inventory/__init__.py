...
def _inventory(args: str = "") -> str:
    ...
    try:
        ...
        result = telegram_intake_router.handle_message("/inventory " + args.strip(), source_link="telegram:6080816249")
        return telegram_intake_router._result_to_text(result)
    except Exception as exc:
        return "Command failed. Check gateway logs. Error: " + str(exc) + "\n" + traceback.format_exc(limit=1)

def register(ctx):
    ctx.register_command("inventory", _inventory, description="Non-Profit Hermes: track inventory.",
