...
def _task(args: str = "") -> str:
    ...
    try:
        ...
        result = telegram_intake_router.handle_message("/task " + args.strip(), source_link="telegram:6080816249")
        return telegram_intake_router._result_to_text(result)
    except Exception as exc:
        return "Command failed. Check gateway logs. Error: " + str(exc) + "\n" + traceback.format_exc(limit=1)

def register(ctx):
    ctx.register_command("task", _task, description="Non-Profit Hermes: create a task.",
